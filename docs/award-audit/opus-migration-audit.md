...
## Roame SkyView instrument onboarded (2026-07-02, night)

> Owner's Roame Pro subscription unlocks a second live instrument covering 31 programmes, incl. ones seats.aero can't reach. Method: URL-parameterized SkyView searches (origin/destination/date-range/searchClass); the collapsed rows' "pts" can encode CREDIT-CARD TRANSFER BONUSES (rows with a "+30%"-style chip show bonus-adjusted card points, not programme prices) — only the expanded card's "book directly with your loyalty account" price is the native programme value, and the Book link's domain identifies the programme.
> - **atmos RESOLVED & FIXED:** SEA–ANC economy 12,500 confirmed as the native Atmos price (direct-booking option links alaskaair.com) — band-3 economy raised 10,000→12,500; band-4 raised 12,500→17,500 (SEA–JFK bottoms 17,500/22,500 across both instruments; 12,500 never observed). LAX–SEA First = exactly 25,000 as charted — the seats.aero "business 20,000" was a cabin-mapping artifact, chart vindicated.
> - Coverage now available for follow-ups: lifemiles (feed empty on seats.aero), and potentially others among Roame's 31 programmes — enumeration of the programme filter pending.
