...
import { ArrowRightLeft, ArrowUp, Check, Copy, FileText } from 'lucide-react'
import {
  Conversation,
  ConversationContent,
...
// Generic composer — accepts `extras` rendered above the text input.
// The editor passes its Upload / Add accounts / Update balance chip row as
// extras; the capture review passes nothing.
function Composer({
  onSubmit,
  status,
  onStop,
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      <PromptInput onSubmit={onSubmit}>
        <PromptInputTextarea placeholder={placeholder ?? 'Ask anything'} />
        <PromptInputFooter>
          <PromptInputTools />
...
export function DraftChat({
  ...
  // If provided, replaces the default empty-state layout. Receives the
  // pre-wired composer node so the caller can embed it wherever it likes
  // (e.g. centred under a hero heading). The composer must be rendered
  // somewhere inside emptyState so the user can still type.
  emptyState?: (composer: React.ReactNode) => React.ReactNode
  placeholder?: string
  footerNote?: React.ReactNode
  onShowInJournal?: (range: { from: string; to: string } | null) => void
  onBusyChange?: (busy: boolean) => void
  onClearableChange?: (state: { canClear: boolean; clear: () => void }) => void
  // Pass-through kept for API compatibility; callers may use it directly.
  ...
  // Run-once guard for the ?prefill= auto-send path. The component mounts
  // after hydration, so this effect fires exactly once per page load. The ref
  // prevents a second send if React ever re-runs effects in strict mode.
  ...
  // Only active when autoPrefill=true (editor only).
  useEffect(() => {
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
      {isEmpty ? (
        emptyState ? (
          <PromptInputProvider>{emptyState(composer)}</PromptInputProvider>
        ) : (
