...
export type PointsPathsResult = {
  // In 'to' (booking) mode this is the destination programme you want to reach.
  // In 'from' (book-from) mode this is the ANCHOR you hold — a programme or a
  // card — that the graph fans out FROM. Same field, mirrored meaning.
...
export async function buildPointsFrom(
  ...
  const edges: PathEdge[] = []
  if (cardAnchor) for (const s of seeds) edges.push({ from: cardAnchor, to: s.program, kind: 'earn' })
  await Promise.all(
    [...programs].map(async (p) => {
      ...
      for (const it of tr.items ?? []) {
        ...
        const rd = Number(it.attrs?.ratio_dest)
        if (!(rs > 0 && rd > 0)) continue
        edges.push({ from: p, to: it.other, kind: 'transfer', ratio_source: rs, ratio_dest: rd, multiplier: rs / rd, transfer_time: tickerStr(it.attrs?.transfer_time) })
      }
    }),
  )
