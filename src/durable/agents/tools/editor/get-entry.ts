...
// kind+id (from a find_entries row). The fetcher is injected (no DO closure).
// raw_text enters context only for the entries the model is about to change —
// the whole point of the find_entries → get_entry → draft split.
export function getEntryTool(
  fetchEntry: (ref: { kind: EntryKindT; id: number }) => Promise<EntryBlob | null>,
) {
  return tool({
    description:
      'Read the full beancount text of ONE existing entry (use ids from a `find_entries` row). ' +
