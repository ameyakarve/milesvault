      ...
      if (images.length > 0) {
        ...
        try {
          const composed = streamText({
            ...
            system:
              'You are given a credit-card statement as layout-extracted TEXT and as page IMAGES. Produce ONE consolidated plain-text copy of the statement that merges both. The TEXT is authoritative for any value present in both — copy its numbers, dates and amounts verbatim, never re-key them from the image. Use the IMAGES to add what the text is missing: content the bank renders as a graphic (styled summary / reward / totals boxes and their labels), written in place WITH its labels. When a SUMMARY / REWARD / TOTALS box is laid out as a table — a row of column LABELS above (or beside) a row of VALUES — attach each value to its own label as `Label: value`, one per line, so NO figure is left as a bare number under a separate header row (e.g. a reward box becomes `Reward Points: <closing> / Opening Balance: <n> / Earned: <n> / …`). This applies ONLY to those summary/reward/totals boxes — leave the per-transaction rows exactly as they are. Keep every transaction and every figure; do not summarise, drop, re-order, draft, or comment. Write COMPACTLY: a single space between fields, never pad with runs of spaces, tabs, or repeated blank lines (no column alignment) — extra whitespace wastes output tokens and carries no meaning. Output only the consolidated statement text.',
            messages: [
              {
                role: 'user',
                content: [
